...
function generate_core {
    ...
    ############### RUN PERFORMANCE ###############
    if [ "$run_perf" == "1" ];then
        if echo "${need_name_list[@]}" | grep -w "${model_name}" &>/dev/null;
        then
            ...
            do
                ...
                printf "numactl --localalloc --physcpubind ${cpu_array[i]} \
                    timeout 7200 python  tf_benchmark.py \
        ...
        elif [[ "${need_disable_optimize_list[@]}" == $model_name ]]
        then
            for(( i=0; i<instance; i++ ))
            do
                real_cores_per_instance=$(echo ${cpu_array[i]} |awk -F, '{print NF}')
                ...
                printf "numactl --localalloc --physcpubind ${cpu_array[i]} \
                    timeout 7200 python  tf_benchmark.py \
                        --num_warmup ${num_warmup} --num_iter ${num_iter} \
                        --model_path $checkpoint \
                        --model_name ${model_name} --disable_optimize\
                        --precision ${precision} \
                > ${log_file} 2>&1 &  \n" |tee -a ${excute_cmd_file}
            done
