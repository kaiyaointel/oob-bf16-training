    ...
    if [ "$run_perf" == "1" ];then
		...
		do
			...
			printf "numactl --localalloc --physcpubind ${cpu_array[i]} \
                timeout 3600 python3 train_predict.py \
                            --train_file ./data/daily-minimum-temperatures-in-me.csv \
...
function collect_logs {
    ...
    echo "${period}; ${model_name}; ${precision};${throughput}; ${compute_op_time}" | tee -a ${workspace}/summary.log
}
