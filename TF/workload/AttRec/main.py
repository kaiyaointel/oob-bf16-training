...
tf.app.flags.DEFINE_string('train','False','train the model')
tf.app.flags.DEFINE_string('eval','True','evaluation the model')
tf.app.flags.DEFINE_integer('num_warmup', 50, 'number of warmup')
tf.app.flags.DEFINE_integer('num_eval_samples', 500, 'number of total eval samples')
...
def main(args):
    ...
    with tf.Session() as sess:
        ...
        if FLAGS.eval == "True":
            ...
            total_time = 0
            index = 0
            for _input in trainIterator:
                user, next_target, user_seq, sl, neg_seq = _input
                feed_dict = {model.u_p: user, model.next_p: next_target, model.sl: sl,
                             model.hist_seq: user_seq,model.keep_prob:1.0
                             ,model.is_Training:False}
            ...
            hr50 = Metric_HR(next_list,pred_list,50)
            mrr = Metric_MRR(next_list,pred_list)
            print(" eval:  test HR@50: {:g}, test MRR: {:g}"
                  .format(hr50, mrr))
        
            print("Total time: {}, Throughput: {}".format(total_time, total_samples / total_time))
