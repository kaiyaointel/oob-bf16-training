...
def train(model, criterion, criterion_st, optimizer, optimizer_st, scheduler,
    ...
    for num_iter, data in enumerate(data_loader):
        ...
        if args.bf16_train_cpu:
            ...
            with torch.cuda.amp.autocast(enabled=True):
                ...
                if c.bidirectional_decoder:
                    ...
                    decoder_output, postnet_output, alignments, stop_tokens = model(
                        text_input, text_lengths.detach().cpu(), mel_input, speaker_ids=speaker_ids)
        ...
        if c.separate_stopnet:
            stop_loss = stop_loss.detach().cpu()
            stop_loss = Variable(stop_loss, requires_grad = True)
            stop_loss.backward()
        ...
        latency = batch_time.avg / c.batch_size * 1000 #kyao
        throughput = c.batch_size / batch_time.avg #kyao
...
if __name__ == '__main__':
    ...
    parser.add_argument('--bf16-train-cpu', action='store_true', help='apply torch.cpu.amp.aumocast to train with bf16 on CPU')
    parser.add_argument('--bf16-train-cuda', action='store_true', help='apply torch.cuda.amp.aumocast to train with bf16 on CUDA')
