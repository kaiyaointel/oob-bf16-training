...
function prepare_workload {
    ...
    patch -p1 < ${workload_dir}/TTS.diff || true
    cp ${workload_dir}/benchmark.py .
    ...
    if [ ! -e models/tacotron2 ];then
        ln -sf /home2/pytorch-broad-models/TTS/models/tacotron2/ models/
        ln -sf /home2/pytorch-broad-models/TTS/datasets/ljspeech/ datasets/
    fi
    if [ $(espeak-ng --help > /dev/null 2>&1 && echo $? || echo $?) -ne 0 ];then
        cp /home2/pytorch-broad-models/TTS/espeak-ng-1.50.tar.gz .
        tar xvf espeak-ng-1.50.tar.gz
        cd espeak-ng-1.50/
        ./autogen.sh
...
function main {
    ...
    do
        # pre run
        python benchmark.py --model-path ./models/tacotron2/best_model.pth.tar --config-path=./models/tacotron2/config.json \
            --dataset-name=ljspeech --metadata-path=./datasets/ljspeech/truncated_metadata.csv \
...
function generate_core {
    ...
    do
        ...
        printf "numactl --localalloc --physcpubind ${cpu_array[i]} timeout 7200 \
            python benchmark.py --model-path ./models/tacotron2/best_model.pth.tar \
                --config-path=./models/tacotron2/config.json --dataset-name=ljspeech \
                --metadata-path=./datasets/ljspeech/truncated_metadata.csv \
...
function collect_perf_logs {
    ...
    # latency=$(grep 'Total Average Throughput:' ${log_dir}/rcpi* |sed -e 's/.*Throughput//;s/,.*//;s/[^0-9.]//g' |awk -v bs=${batch_size} '
    #     BEGIN {
    #         sum = 0;
    #         i = 0;
    #     }
    ...
    #     }
    # ')
    throughput=$(grep 'Total Average Throughput:' ${log_dir}/rcpi* |sed -e 's/.*Throughput//;s/,.*//;s/[^0-9.]//g' |awk '
        BEGIN {
            sum = 0;
        }
        {
