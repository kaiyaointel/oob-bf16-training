...
function prepare_workload {
    ...
    patch -p1 < ${workload_dir}/wide_deep.patch || true
    cp ${workload_dir}/conf.yaml .
    ...
    if [ ! -e model ];then
        ln -sf /home2/pytorch-broad-models/widedeep/data/ .
        ln -sf /home2/pytorch-broad-models/widedeep/model/ .
    fi
}
...
function main {
    ...
    do
        # pre run
        python ./inference_arch1_binary.py --inf --eval --num_warmup 1 --num_iter 2 --batch_size 1 \
...
function generate_core {
    ...
    do
        ...
        printf "numactl --localalloc --physcpubind ${cpu_array[i]} timeout 7200 \
            python ./inference_arch1_binary.py --inf --eval \
