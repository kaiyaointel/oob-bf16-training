...
def pass_epoch(
    ...
    for i_batch, (x, y) in enumerate(loader):
        ...
        loss_batch = Variable(loss_batch, requires_grad = True)
        if model.training:
            loss_batch.backward()
            optimizer.step()
            optimizer.zero_grad()
        ...
        if writer is not None and model.training:
            if writer.iteration % writer.interval == 0:
                writer.add_scalars('loss', {mode: loss_batch.float().detach().cpu()}, writer.iteration)
    ...
    if writer is not None and not model.training:
        writer.add_scalars('loss', {mode: loss.float().detach()}, writer.iteration)
