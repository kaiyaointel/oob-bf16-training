...
function main {
    ...
    if [ ! -e VOC2007 ];then
        ln -sf /home2/pytorch-broad-models/ssd-vgg16/model/* models/
        ln -sf /home2/pytorch-broad-models/VOC2007/ .
    fi
    ...
    do
        # vgg16-ssd,mb1-ssd,mb2-ssd-lite
        if [ "${model_name}" == "mb1-ssd" ];then
            model_path="./models/mobilenet-v1-ssd-mp-0_675.pth"
        elif [ "${model_name}" == "mb2-ssd-lite" ];then
            model_path="./models/mb2-ssd-lite-mp-0_686.pth"
        elif [ "${model_name}" == "vgg16-ssd" ];then
            model_path="./models/vgg16-ssd-mp-0_7726.pth"
        fi
        # pre run
        python eval_ssd.py --net ${model_name} --dataset ./VOC2007/ --trained_model ${model_path} \
            --label_file ./models/voc-model-labels.txt --model_name ${model_name} --num_iters 2 --num_warmup 1 \
...
function init_params {
    ...
    #
    if [ "${model_name}" == "pytorch-ssd" ];then
        model_name="vgg16-ssd,mb1-ssd,mb2-ssd-lite"
    fi
}
...
function generate_core {
    ...
    do
        ...
        printf "numactl --localalloc --physcpubind ${cpu_array[i]} timeout 7200 \
            python eval_ssd.py --net ${model_name} --model_name ${model_name} --trained_model ${model_path} \
                --dataset ./VOC2007/ --label_file ./models/voc-model-labels.txt \
