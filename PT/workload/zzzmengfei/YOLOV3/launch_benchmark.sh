...
function prepare_workload {
    ...
    if [ ! -e YOLOV3 ];then
        ln -sf /home2/pytorch-broad-models/YOLOV3/ .
    fi
}
...
function main {
    ...
    do
        ...
        if [ "${model_name}" == "YoloV3" ];then
            model_weight="./YOLOV3/models/yolov3.weights"
            model_config="./config/yolov3.cfg"
        elif [ "${model_name}" == "YoloV3-tiny" ];then
            model_weight="./YOLOV3/models/yolov3-tiny.weights"
            model_config="./config/yolov3-tiny.cfg"
        fi
        # pre run
        python detect.py --image_folder ./data/samples/ --arch ${model_name} --weights_path ${model_weight} --model_def ${model_config} \
...
function generate_core {
    ...
    do
        ...
        printf "numactl --localalloc --physcpubind ${cpu_array[i]} timeout 7200 \
            python detect.py --arch ${model_name} --image_folder ./data/samples/ \
                --arch ${model_name} --weights_path ${model_weight} --model_def ${model_config} \
