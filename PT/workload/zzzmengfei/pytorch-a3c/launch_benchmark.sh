...
function main {
    ...
    if [ ! -d ROMS ];then
        rsync -avz /home2/pytorch-broad-models/pytorch-a3c/ROMS.zip .
        unzip ROMS.zip
    fi
    ...
    do
        # pre run
        python main.py --env-name "PongDeterministic-v4" --arch A3C \
...
function collect_perf_logs {
    #
    # latency=$(grep 'Throughput is:' ${log_dir}/rcpi* |sed -e 's/.*Throughput//;s/[^0-9.]//g' |awk -v bs=${batch_size} '
    #     BEGIN {
    #         sum = 0;
    #         i = 0;
    #     }
    ...
    #     }
    # ')
    throughput=$(grep 'Throughput is:' ${log_dir}/rcpi* |sed -e 's/.*Throughput//;s/[^0-9.]//g' |awk '
        BEGIN {
            sum = 0;
        }
        {
