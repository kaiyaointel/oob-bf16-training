...
function main {
    # prepare workload
    workload_dir="${PWD}"
    source_code_dir="../../bert-large/"
    init_model ${source_code_dir}
    #
    patch -p1 < ${workload_dir}/bert_large.patch || true
    ...
    do
        # pre run
        python examples/text-classification/run_glue.py --max_seq_length 128 --do_eval \
            --task_name MRPC --data_dir ./glue_data/MRPC/ --output_dir ./mrpc_output/ \
            --model_name_or_path bert-large-cased --arch ${model_name} \
            --per_device_eval_batch_size 1 --num_iters 2 --num_warmup_iters 1 \
    ...
    do
        ...
        printf "numactl --localalloc --physcpubind ${cpu_array[i]} timeout 7200 \
            python examples/text-classification/run_glue.py --max_seq_length 128 --do_eval \
                --task_name MRPC --data_dir ./glue_data/MRPC/ --output_dir ./mrpc_output/ \
                --model_name_or_path bert-large-cased --arch ${model_name} \
                --num_iters ${num_iter} --num_warmup_iters ${num_warmup} \
