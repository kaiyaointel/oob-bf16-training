...
function main {
    ...
    patch -p1 < ${workload_dir}/NCF.patch || true
    ...
    cd recommendation/
    if [ ! -e dataset ];then
        ln -sf /home2/pytorch-broad-models/NCF/dataset/ .
    fi
    ...
    do
        # pre run
        python pytorch/ncf.py ./dataset/ml-20mx1x1 --layers 256 256 128 64 -f 64 --seed 1 --user_scaling 1 \
            --item_scaling 1 --random_negatives --evaluate -b 1 --warmup-iter 1 --num-iter 2 \
...
function generate_core {
    ...
    do
        ...
        printf "numactl --localalloc --physcpubind ${cpu_array[i]} timeout 7200 \
            python pytorch/ncf.py ./dataset/ml-20mx1x1 --layers 256 256 128 64 \
                -f 64 --seed 1 --user_scaling 1 \
