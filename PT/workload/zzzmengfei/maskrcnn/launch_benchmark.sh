...
function prepare_workload {
    ...
    patch -p1 < ${workload_dir}/maskrcnn.diff || true
    ...
    if [ ! -d datasets ];then
        mkdir -p datasets
        ln -sf /home2/pytorch-broad-models/COCO2014/ datasets/coco
    fi
    if [ ! -e cocoapi ];then
...
function main {
    ...
    do
        # pre run
        python ./tools/test_net.py --precision ${precision} --channels_last ${channels_last} \
            --config-file 'configs/e2e_mask_rcnn_R_50_FPN_1x.yaml' --warmup 1 DATALOADER.NUM_WORKERS 1 TEST.IMS_PER_BATCH 1 \
            MODEL.DEVICE cpu PER_EPOCH_EVAL True SOLVER.MAX_ITER 2 || true
        #
...
function generate_core {
    ...
    do
        ...
        printf "numactl --localalloc --physcpubind ${cpu_array[i]} timeout 7200 \
            python ./tools/test_net.py ${addtion_options} \
                --precision ${precision} --channels_last ${channels_last} \
                --config-file 'configs/e2e_mask_rcnn_R_50_FPN_1x.yaml' --warmup 5 \
                DATALOADER.NUM_WORKERS 1 TEST.IMS_PER_BATCH ${batch_size} \
                MODEL.DEVICE cpu PER_EPOCH_EVAL True SOLVER.MAX_ITER 20 \
...
function collect_perf_logs {
    ...
    # latency=$(grep 'Model inference Throughput:' ${log_dir}/rcpi* |sed -e 's/.*Throughput//;s/,.*//;s/[^0-9.]//g' |awk -v bs=${batch_size} '
    #     BEGIN {
    #         sum = 0;
    #         i = 0;
    #     }
    ...
    #     }
    # ')
    throughput=$(grep 'Model inference Throughput:' ${log_dir}/rcpi* |sed -e 's/.*Throughput//;s/,.*//;s/[^0-9.]//g' |awk '
        BEGIN {
            sum = 0;
        }
        {
