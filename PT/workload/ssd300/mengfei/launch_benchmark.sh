...
function main {
    ...
    if [ ! -e COCO2017 ];then
        ln -sf /home2/pytorch-broad-models/COCO2017/ .
    fi
    ...
    do
        # pre run
        python train.py --epochs 1 --warmup-factor 0 --lr 2.5e-3 --no-save --threshold=0.23 --data ./COCO2017 \
            --evaluation 1  --eval-only --perf-prerun-warmup 1 --arch ssd300 -b 1 --perf-run-iters 2 \
...
function generate_core {
    ...
    do
        ...
        printf "numactl --localalloc --physcpubind ${cpu_array[i]} timeout 7200 \
            python train.py --epochs 1 --warmup-factor 0 --lr 2.5e-3 --no-save --arch ssd300 \
                --threshold=0.23 --data ./COCO2017 --evaluation 1  --eval-only \
                --perf-prerun-warmup ${num_warmup} --perf-run-iters ${num_iter} \
                -b ${batch_size} \
                --precision ${precision} \
